...
class Text extends Element {
    constructor(text, args) {
        let {family, size, actual, style, ...attr} = args ?? {};
        size = size ?? font_size_base;
        style = style ?? true;
        actual = actual ?? false;

        let font_calc = family ?? font_family_base;
        let font_attr = style ? font_calc : undefined;
        ...
        let attr1 = {aspect: aspect, font_family: font_attr, ...attr};
...
class Node extends Container {
    constructor(text, args) {
        ...
        if (aspect == null) {
            let [px, py] = is_scalar(padding) ? [padding/text.aspect, padding] : padding;
...
class SymPath extends Element {
    constructor(args) {
        let {fy, fx, xlim, ylim, tlim, N, ...attr} = args ?? {};
        N = N ?? 100;
        ...
        this.xlim = xlim ?? [Math.min(...xvals), Math.max(...xvals)];
        this.ylim = ylim ?? [Math.min(...yvals), Math.max(...yvals)];

        let [xmin, xmax] = this.xlim;
        let [ymin, ymax] = this.ylim;
        let xrange = xmax - xmin;
        let yrange = ymax - ymin;

        this.xnorm = (xrange != 0) ? xvals.map(x => (x-xmin)/xrange) : ensure_vector(0.5, N);
        this.ynorm = (yrange != 0) ? yvals.map(y => (y-ymin)/yrange) : ensure_vector(0.5, N);
    }
...
function make_tick(s, prec) {
    return new Text(rounder(s, prec), {
        family: 'Montserrat', font_weight: 100
    });
}

function ensure_tick(t, prec) {
    if (is_scalar(t)) {
        return [t, make_tick(t, prec)];
    } else {
        let [p, x] = t;
        if (x instanceof Element) {
            return t;
        } else {
            return [p, make_tick(x, prec)];
        }
    }
}
...
let Gum = [
    ...
    HLine, VLine, Rect, Square, Ellipse, Circle, Polyline, Polygon, Path, Text, Node, MoveTo,
    LineTo, Bezier2, Bezier3, Arc, Close, SymPath, XScale, YScale, XAxis, YAxis, Axes, Plot, range,
    linspace, exp, log, sin, cos, min, max
];
...
export {
    ...
    HStack, Point, Scatter, Spacer, Ray, Rect, Square, Ellipse, Circle, Polyline, Polygon, Path,
    Text, Node, MoveTo, LineTo, Bezier2, Bezier3, Arc, Close, SymPath, XScale, YScale, XAxis, YAxis,
    Axes, Plot
};
