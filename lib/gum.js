...
class SymPoints extends Container {
    constructor(args) {
        ...
        let children = points.map(p =>
            [fs(...p), rad_rect(p, fr(...p))]
        );
...
class Plot extends Container {
    constructor(lines, args) {
        let {
            ...
            title, ticksize, axislabelsize, ticklabelsize, titlesize, prec, aspect, margin, ...attr
        } = args ?? {};
        ...
        if (xlabel != null) {
            ...
            let xlabeloffset = 3*axes.xticksize;
            let xlabelrect = [0, 1+xlabeloffset, 1, 1+xlabeloffset+xaxislabelsize];
            children.push([xlabel, xlabelrect]);
        }
        if (ylabel != null) {
            ylabel = new YLabel(ylabel);
            let ylabeloffset = 5*axes.yticksize;
            let ylabelrect = [-ylabeloffset-yaxislabelsize, 0, -ylabeloffset, 1];
...
export {
    ...
    LineTo, Bezier2, Bezier3, Arc, Close, SymPath, SymPoints, Scatter, XScale, YScale, XAxis,
    YAxis, Axes, Graph, Plot, InterActive, Variable, Slider, Toggle, List, XTicks, YTicks,
