...
class XAxis extends Container {
    constructor(ticks, args) {
        ...
        let scale = new XScale(ticks, anchor);
        let children = [scale];
        super(children, attr);
    }
}
...
class YAxis extends Container {
    constructor(ticks, args) {
        let {anchor, ...attr} = args ?? {};
        let scale = new YScale(ticks, anchor);
        let children = [scale];
        super(children, attr);
    }
}
...
class Plot extends Container {
    constructor(lines, args) {
        let {xlim, ylim, xticks, yticks, ticksize, aspect, ...attr} = args ?? {};
        xticks = xticks ?? 10;
        yticks = yticks ?? 10;
        ...
        xticks = (!isNaN(xticks)) ? linspace(xmin, xmax, xticks+1).slice(1) : xticks;
        yticks = (!isNaN(yticks)) ? linspace(ymin, ymax, yticks+1).slice(1) : yticks;
