...
class Plot extends Container {
    constructor(lines, args) {
        let {
            ...
            ticksize, labelsize, prec, aspect, margin, ...attr
        } = args ?? {};
        ...
        let axes = new Axes({
            ...
            xlim: graph.xlim, ylim: graph.ylim, aspect: graph.aspect, prec: prec
        });

        // create labels and title
        let labels = [];
        let [xlabelsize, ylabelsize] = aspect_invariant(labelsize, graph.aspect);

        if (xlabel != null) {
            xlabel = new XLabel(xlabel);
            let xlabeloff = 3*axes.xticksize;
            let xlabelrect = [0, 1+xlabeloff, 1, 1+xlabeloff+xlabelsize];
        ...
        if (ylabel != null) {
            ...
            let ylabelrect = [-ylabeloff-ylabelsize, 0, -ylabeloff, 1];
