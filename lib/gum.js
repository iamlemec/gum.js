...
function SideRenderTextSizer(html, args) {
    ...
    x = 0;
    y = 0;
    w = textDiv.getBoundingClientRect().width;
    h = textDiv.getBoundingClientRect().height;
...
class Tex extends Element {
    constructor(text, args) {
        ...
        // pass to element
        let attr1 = {aspect: aspect, width: width, height: height, ...attr};
    ...
    props(ctx) {
        ...
        let base = {x: x, y: y, font_size: `${this.fontsize}px`};
        return {...base, ...this.attr};
    }
...
class Slider extends Variable{
    ...
    anchor(name, targ, ctx, redraw){ //ctx is an interactive context
        let v = this;

        let input = document.createElement("input");
        input.type = "range";
        input.className = "slider"; // set the CSS class
        input.id = `InterActive_${name}`
        targ.appendChild(input); // put it into the DOM
...
let Gum = [
    ...
    Plot, range, linspace, exp, log, sin, cos, min, max, InterActive, Variable, Slider
];
...
export {
    ...
    XAxis, YAxis, Axes, Plot, InterActive, Variable, Slider
};
