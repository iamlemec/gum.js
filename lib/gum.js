...
class Container extends Element {
    ...
    inner(ctx) {
        ...
        /*
        let rects0 = this.children.map(([c, r]) => [r[0], r[1]]);
        let rects1 = this.children.map(([c, r]) => [r[2], r[3]]);
        rects0 = ctx.coord_to_frac(rects0);
        rects1 = ctx.coord_to_frac(rects1);
        let rects = zip(rects0, rects1).map(([r0, r1]) => [...r0, ...r1]);
        let children = zip(this.children, rects).map(([[c, r0], r]) => [c, r]);
        */
...
class InterActive {
    ...
    createAnchors(targ, redraw){ //tag is where to append anc, redraw is where to redraw
        let i = this;
        Object.entries(this.vars).forEach(function([v, k]) {
...
function updateSliderValue(slider) {
    let pos = slider.value / slider.max;
...
let Gum = [
    ...
    Graph, Plot, range, linspace, exp, log, sin, cos, min, max, sqrt, InterActive, Variable, Slider
];
...
export {
    ...
    linspace, exp, log, sin, cos, min, max, sqrt, Context, Element, Container, Group, SVG, Frame,
    VStack, HStack, Place, Points, Spacer, Ray, Rect, Square, Ellipse, Circle, Polyline, Polygon,
    Path, Text, Tex, Node, MoveTo, LineTo, Bezier2, Bezier3, Arc, Close, SymPath, Scatter, XScale,
    YScale, XAxis, YAxis, Axes, Graph, Plot, InterActive, Variable, Slider
};
