...
try {
    ...
    console.log(textSizer('hello world'));
} catch (error) {
    console.log(error);
}
...
class Text extends Element {
    constructor(text, args) {
        let {family, size, ...attr} = args ?? {};

        let fargs = {family: family, size: size};
        let [width, height] = textSizer(text, fargs);
    ...
    props(ctx) {
        ...
        let base = {x: x1, y: y2, font_size: `${h}px`};
        return {...base, ...this.attr};
    }
...
let Gum = [
    ...
    HLine, VLine, Rect, Square, Ellipse, Circle, Text, range, linspace
];
...
export {
    ...
    Spacer, Ray, Rect, Square, Ellipse, Circle, Text
};
