...
class Scatter extends Container {
    constructor(points, args) {
        ...
        // pass to container
        let children = points.map(p => [shape, rad_rect(p, radius)]);
        let attr1 = {clip: false, ...attr};
        super(children, attr1);
    }
}
...
class Axes extends Container {
    constructor(args) {
        let {xticks, yticks, xsize, ysize, xlim, ylim, ...attr} = args ?? {};
        xticks = xticks ?? [];
        yticks = yticks ?? [];
...
class Plot extends Container {
    constructor(lines, args) {
        let {xlim, ylim, xticks, yticks, ticksize, prec, aspect, margin, ...attr} = args ?? {};
        xticks = xticks ?? 5;
        yticks = yticks ?? 5;
        ticksize = ticksize ?? tick_size_base;

        // create actual graph from lines and pick up ranges and aspect
        let graph = new Graph(lines, {xlim: xlim, ylim: ylim, aspect: aspect});
        let scale = graph.scale;
        aspect = graph.aspect;

        // extract limits
        let [xmin, ymax, xmax, ymin] = scale;
        xlim = xlim ?? [xmin, xmax];
        ylim = ylim ?? [ymin, ymax];
        ylim = ylim.reverse(); // shit's inverted
        ...
        // create axes
        let axes = new Axes({
            xticks: xticks, yticks: yticks, xsize: xticksize, ysize: yticksize, xlim: xlim, ylim: ylim
        });
...
class Radio extends Variable {
    ...
    anchor(name, targ, ctx, redraw) {
        ...
        Object.entries(Object.entries(v.attr.choices)).forEach(([index, [label, value]]) => {
            radioInner += `<input type="radio" class=radio__input id="Radio_${name}_${index}" name="Radio_${name}" value="${value}">
...
let Gum = [
    ...
    Axes, Graph, Plot, range, linspace, zip, exp, log, sin, cos, min, max, sqrt, floor, ceil,
    round, pi, InterActive, Variable, Slider, Toggle, Radio, XTicks, rounder, make_ticklabel, XTicks,
    YTicks
];
...
export {
    Gum, gzip, zip, pos_rect, pad_rect, rad_rect, demangle, rounder, props_repr, range,
    linspace, exp, log, sin, cos, min, max, sqrt, floor, ceil, round, pi, Context, Element,
    Container, Group, SVG, Frame, VStack, HStack, Place, Points, Spacer, Ray, Line, Rect, Square,
    Ellipse, Circle, Polyline, Polygon, Path, Text, Tex, Node, MoveTo, LineTo, Bezier2, Bezier3,
    Arc, Close, SymPath, Scatter, XScale, YScale, XAxis, YAxis, Axes, Graph, Plot, InterActive,
    Variable, Slider, Toggle, Radio, XTicks, rounder, make_ticklabel, XTicks, YTicks
};
