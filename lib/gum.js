...
class Command {
    ...
    data(ctx) {
        let [x1, y1, x2, y2] = ctx.rect;
        let [w, h] = [x2 - x1, y2 - y1];
        let pts = this.data.map(([fx, fy]) => [x1 + w*fx, y1 + h*fy]);
        return pts.join(',');
    }

    string(ctx) {
        let dat = this.data(ctx);
        return `${this.cmd} ${dat}`;
    }
}
...
let Gum = [
    ...
    HLine, VLine, Rect, Square, Ellipse, Circle, Polyline, Polygon, Path, Text, Node, range,
    linspace
];
...
export {
    ...
    Spacer, Ray, Rect, Square, Ellipse, Circle, Polyline, Polygon, Path, Text, Node
};
