...
class Scatter extends Container {
    constructor(points, args) {
        let {radius, shape, xlim, ylim, fill, ...attr} = args ?? {};
        fill = fill ?? 'black'
        shape = shape ?? new Circle({fill: fill});
        radius = radius ?? 0.05;
...
class Variable {
    constructor(init, args) {
        args = args ?? {};
        this.value = init;
        this.attr = Object.fromEntries(Object.entries(args).filter(([k, v]) => v != null));
    }
