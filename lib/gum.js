...
class XAxis extends Container {
    constructor(ticks, args) {
        let {anchor, ...attr} = args ?? {};
        let line = new HLine(anchor);
        let scale = new XScale(ticks);
        let children = [line, scale];
        super(children, attr);
    }
}
...
class Plot extends Container {
    constructor(lines, args) {
        let {xlim, ylim, xticks, yticks, xticksize, yticksize, ...attr} = args ?? {};
        ...
        let ymap = y => (y-ymin)/(ymax-ymin);

        // map lines into coordinates
        let coords = zip(xmins, xmaxs, ymins, ymaxs).map(
            ([x1, x2, y1, y2]) => [xmap(x1), 1-ymap(y2), xmap(x2), 1-ymap(y1)]
        );

        // handle integer tick specifier
        xticks = (!isNaN(xticks)) ? linspace(xmin, xmax, xticks) : xticks;
        yticks = (!isNaN(yticks)) ? linspace(ymin, ymax, yticks) : yticks;

        // map ticks into coordinates
        xticks = (xticks != null) ? xticks.map(xmap) : null;
        yticks = (yticks != null) ? yticks.map(ymap) : null;
...
let Gum = [
    ...
    LineTo, Bezier2, Bezier3, Arc, Close, SymPath, YScale, XAxis, YAxis, Axes, Plot, range,
...
export {
    ...
    Text, Node, MoveTo, LineTo, Bezier2, Bezier3, Arc, Close, SymPath, YScale, XAxis, YAxis,
    Axes, Plot
};
