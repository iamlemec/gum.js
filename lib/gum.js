...
function make_ticklabel(s, prec) {
    ...
    return new Text(t, {
        calc_family: plot_font_base, font_weight: 100
    });
}

function make_axislabel(s) {
    return new Text(s, {
        calc_family: plot_font_base, font_weight: 100
    });
}
...
class Axes extends Container {
    constructor(args) {
        let {
            xticks, yticks, ticksize, labelsize, xlim, ylim, xanchor, yanchor, aspect, prec, ...attr
        } = args ?? {};
        ...
        // create axes
        let xaxis = (xticks != null) ? new XAxis(xticks, {lim: xlim, labelsize: labelsize}) : null;
        let yaxis = (yticks != null) ? new YAxis(yticks, {lim: ylim, labelsize: labelsize}) : null;
...
class XLabel extends Container {
    constructor(text, attr) {
        let label = is_element(text) ? text : new make_axislabel(text);
...
class Plot extends Container {
    constructor(lines, args) {
        let {
            xlim, ylim, xticks, yticks, xanchor, yanchor, xlabel, ylabel,
            ticksize, axislabelsize, ticklabelsize, prec, aspect, margin, ...attr
        } = args ?? {};
        axislabelsize = axislabelsize ?? axis_label_size_base;
        ...
        let labels = [];
        let [xaxislabelsize, yaxislabelsize] = aspect_invariant(axislabelsize, graph.aspect);

        if (xlabel != null) {
            xlabel = new XLabel(xlabel);
            let xlabeloff = 3*axes.xticksize;
            let xlabelrect = [0, 1+xlabeloff, 1, 1+xlabeloff+xaxislabelsize];
            labels.push([xlabel, xlabelrect]);
        }

        if (ylabel != null) {
            ylabel = new YLabel(ylabel);
            let ylabeloff = 3*axes.yticksize;
            let ylabelrect = [-ylabeloff-yaxislabelsize, 0, -ylabeloff, 1];
            labels.push([ylabel, ylabelrect]);
        }

        // get children
        let children = [graph, axes, ...labels];
        children.push();
...
class List extends Variable {
    ...
    anchor(name, targ, ctx, redraw) {
        ...
        targ.appendChild(cont).append(title, list); // slider in cont in targ!
