...
class Grid extends Container {
    constructor(args) {
        let {xgrid, ygrid, xlim, ylim, ...attr} = args ?? {};
        xlim = xlim ?? limit_base;
        ylim = ylim ?? limit_base;
        ...
        if (xgrid != null) {
            let xgridlines = new XScale(xgrid, {ylim: ylim, stroke: '#CCC'});
            grids.push(xgridlines);
        }
        if (ygrid != null) {
            let ygridlines = new YScale(ygrid, {xlim: xlim, stroke: '#CCC'});
...
class Plot extends Container {
    constructor(lines, args) {
        let {
            ...
            ticksize, axislabelsize, ticklabelsize, titlesize, prec, aspect, margin, ...attr
        } = args ?? {};
        ...
        let grid = new Grid({
            xgrid: xgrid, ygrid: ygrid, xlim: graph.xlim, ylim: graph.ylim, aspect: graph.aspect
        });

        // construct optional elements
        let children = [grid, graph, axes];
